...
const InitialSetup = async (treasuryWalletPK, buyerPK, bankBWalletPK, bankATokenAddress, bankBTokenAddress) => {
    try {
        ...
        console.log(`Approve marketplace for admin wallet on token A success! \u2705 `);

        await tokenA
            .connect(treasuryWallet)
            .approve(marketplace.getAddress(), ethers.MaxUint256);
        console.log(`Approve marketplace for treasury wallet (${treasuryWallet.address}) on token A success! \u2705 `);

        await tokenA
            .connect(buyerWallet)
            .approve(marketplace.getAddress(), ethers.MaxUint256);
        console.log(`Approve marketplace for buyer wallet (${buyerWallet.address}) on token A success! \u2705 `);

        await tokenB
            .connect(bankBWallet)
            .approve(marketplace.getAddress(), ethers.MaxUint256);
        console.log(`Approve marketplace for buyer wallet from bank B (${bankBWallet.address}) on token B success! \u2705 `);

        await assetCollection
            .connect(buyerWallet)
            .setApprovalForAll(marketplace.getAddress(), true);
        console.log(`Approve marketplace for buyer wallet (${buyerWallet.address}) on asset contract success! \u2705 `);

        await assetCollection
            .connect(bankBWallet)
            .setApprovalForAll(marketplace.getAddress(), true);
        console.log(`Approve marketplace for buyer wallet from bank B (${bankBWallet.address}) on asset contract success! \u2705 `);

        await assetCollection.grantRole(MarketplaceAccess, marketplace.getAddress());
        console.log(`Grant marketplace access role for marketplace on asset contract success! \u2705 `);
...
const CreateAsset = async (adminWalletAddress, id, price, apr, due) => {
    try {
        ...
        console.log(`Create Invoice (price: ${price}, apr: ${apr}%, Min. fraction: 10%, Due: ${dueTimestamp}) success! \u2705 `);
...
const BuyFromBankA = async (buyerPK, owner, id, fractions) => {
    try {
        ...
        await marketplace.connect(buyerWallet).buy(
            0,
            1,
            id,
            fractions,
        ...
        console.log(`Buy Invoice with ${buyerWallet.address} from Bank A (Asset id: ${id}, fractions: ${fractions / 100}%) success! \u2705 `);
...
const BuyFromBankB = async (buyerPK, owner, id, fractions) => {
    try {
        ...
        console.log(`Buy Invoice with ${buyerWallet.address} from Bank B (Asset id: ${id}, fractions: ${fractions / 100}%) success! \u2705 `);
...
const RelistAsset = async (ownerPK, id, salePrice, fractions) => {
    try {
        ...
        console.log(`Relist Invoice with ${ownerWallet.address} (Asset id: ${id}, Sale price: ${salePrice}, Min. fraction to buy: ${fractions / 100}%) success! \u2705 `);
...
const SettleAsset = async (id, owner) => {
    try {
        ...
        await marketplace
            .settleAsset(id, owner);

        console.log(`Settle Invoice for ${owner} owner (Asset id: ${id}) success! \u2705 `);
